...
class ValidationTest {
    ...
    void testAdvanceValidate() {
        Address address1 = new Address("Germany", "Berlin", "Lützowufer", "27-30", "2");
        Map<String, List<String>> result1 = Validator.advancedValidate(address1);
        Map<String, List<String>> expected1 = Map.of();
        assertThat(result1).isEqualTo(expected1);

        Address address2 = new Address("UK", "London", null, "25", null);
        Map<String, List<String>> result2 = Validator.advancedValidate(address2);
        Map<String, List<String>> expected2 = Map.of("country",  List.of("length less than 4"),
                "street", List.of("cannot be null"));
        assertThat(result2).isEqualTo(expected2);
    }
    // END
}
